...
import {
  ...
  landingCardWrapper,
  type LandingCardContainerVariantProps,
} from "@faiz-ui/theme";
...
export function useLandingCard(props: UseLandingCardProps) {
  ...
  const landingCardContainerClassNames = React.useMemo(
    () => landingCardContainer({ color }),
    [color],
  );
  const landingCardWrapperClassNames = React.useMemo(
    () => landingCardWrapper(),
    [],
  );
  ...
  const landingCardContainerStyles = {
    background:
      " radial-gradient(250px circle at var(--x) var(--y),var(--tw-gradient-from) 0,transparent 100%)",
